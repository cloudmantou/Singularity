...
AI review is an evidence-bound recommendation layer over those existing queues, never a fact-writing path. `shadow` records evaluations without an apply action; `suggest` always requires the authenticated owner to apply a recommendation. The built-in unattended `auto_low_risk` path is limited to exact-hash memory duplicates whose Scope, Vault, evidence root, source class, source time, observation time, Claim state, and Parent state all match. The explicitly enabled external-evolution policy can additionally apply semantic memory consolidation only after strict context/confidence guards and an independent server-side verification. Conflicts and entity identity decisions remain human-gated. Jobs are leased and recoverable, model runs and application receipts are immutable, and application receipts commit in the same database batch as the underlying domain decision. Persisted manifests contain evidence hashes and internal references rather than raw memory text, so normal deletion does not leave a second immutable copy of the source content.
...
An authenticated external MCP agent can participate in memory-candidate review through
`evolution_next` and `evolution_submit`. Singularity remains the queue and mutation
authority: it issues a single hashed lease, exposes only a bounded model-safe Claim
snapshot, rejects stale or replay-conflicting submissions, persists the immutable AI
Review Run, obtains a second opinion from the server-configured verifier, and invokes
the existing Knowledge Evolution coordinator only after all server-side guards pass.
The external agent cannot provide its own verification result. Interrupted eligible
runs are reconciled before a new lease is issued. The resulting Claim remains linked
to its original Evidence Roots, and the source Observations are never edited or deleted
by this workflow. Cross-Vault, cross-Scope, cross-Project, and context-incomplete
candidates never expose Claim content through an external lease. This personal MCP
surface is explicitly enabled with `EXTERNAL_EVOLUTION_MCP=1`; clients receive access
only after the owner approves the existing AUTH_TOKEN-backed OAuth flow.
